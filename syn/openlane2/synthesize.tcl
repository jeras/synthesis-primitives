...
write_json "$::env(STEP_DIR)/proc.json"
exec netlistsvg "$::env(STEP_DIR)/proc.json" -o "$::env(STEP_DIR)/proc.svg"

if { $::env(SYNTH_NO_FLAT) != 1 } {
    flatten
}
write_json "$::env(STEP_DIR)/flatten.json"
exec netlistsvg "$::env(STEP_DIR)/flatten.json" -o "$::env(STEP_DIR)/flatten.svg"
if { $::env(SYNTH_ELABORATE_ONLY) } {
    ...
    exec netlistsvg "$::env(SAVE_NETLIST).json" -o "$::env(SAVE_NETLIST).svg"
    exit 0
}
...
write_json "$::env(STEP_DIR)/alumacc.json"
exec netlistsvg "$::env(STEP_DIR)/alumacc.json" -o "$::env(STEP_DIR)/alumacc.svg"
...
exec netlistsvg "$::env(STEP_DIR)/primitive_techmap.json" -o "$::env(STEP_DIR)/primitive_techmap.svg"
...
catch {show -format dot -colors 1 -width -prefix $::env(STEP_DIR)/techmap}
write_json "$::env(STEP_DIR)/techmap.json"
exec netlistsvg "$::env(STEP_DIR)/techmap.json" -o "$::env(STEP_DIR)/techmap.svg"
...
proc run_strategy {output script strategy_name {postfix_with_strategy 0}} {
    ...
    exec netlistsvg "$output.json" -o "$output.svg"
    design -reset
}
